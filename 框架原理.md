...
UE是建立在UObject对象系统上的，所以引擎里别的模块想要启动加载起来，就得先把CoreUObject模块初始化完成。因此引擎循环的预初始化部分就得开始加载CoreUObject了。
...
InitUOject**→**StaticUObjectInit→UObjectBaseInit→UObjectProcessRegistrants→UObjectForceRegistration→UObjectBase::DeferredRegister
